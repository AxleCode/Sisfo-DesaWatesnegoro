                ...
                <div class="row mb-4">
                    <div class="col-12">
                        <div class="d-flex justify-content-between align-items-center">
                            <h2 class="mb-0">Manajemen Instagram Reels</h2>
                            <a href="{{ route('admin.instagram.tambah') }}" class="btn btn-primary">
                                <i class="fab fa-instagram me-2"></i>Tambah Instagram Reel
                            </a>
                        </div>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>
                                                <a href="{{ $reel->url }}" target="_blank" class="text-decoration-none">
                                                    {{ Str::limit($reel->url, 60) }}
                                                    <i class="fas fa-external-link-alt ms-1"></i>
                                                </a>
                                            </td>
                                            <td>
                                                <span class="badge bg-{{ $reel->is_active ? 'success' : 'secondary' }}">
                                                    {{ $reel->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                                </span>
                                            </td>
                                            <td>{{ $reel->order }}</td>
                                            <td>
                                                <small>{{ $reel->created_at->format('d M Y') }}</small>
                                                <br>
                                                <small class="text-muted">{{ $reel->created_at->format('H:i') }}</small>
                                            </td>
                                            <td>
                                                <div class="btn-group">
                                                    <a href="{{ route('admin.instagram.edit', $reel) }}" 
                                                       class="btn btn-sm btn-primary">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form action="{{ route('admin.instagram.hapus', $reel) }}" 
                                                          method="POST" class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger" 
                                                                onclick="return confirm('Apakah Anda yakin ingin menghapus Instagram reel ini?')">
                                        ...
                                        <tr>
                                            <td colspan="6" class="text-center py-4">
                                                ...
                                                <p>Belum ada Instagram reel. Tambahkan reel pertama Anda.</p>
                                            </td>
                                        </tr>
                                        @endforelse
                                    </tbody>
